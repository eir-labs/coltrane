...
| `vitest_test_path` | `tests/spec_turn_loop.test.ts` (28 laws), with its contract in `tests/spec_turn_loop_fixtures.ts` |
| `stop_condition` | all 28 laws green; `tests/spec_completions_invoker.test.ts` still green without edits to its laws; `npm test` green at the bumped pins; `tsc --noEmit` clean; the reachability and declared-field ratchets not raised |
| `non_goals` | see below |
...
| 5–7 | cost comes from a price table the deployment supplies, keyed by the **served** model. An unpriced model or token class is *unpriced*; a round with no usage is *unreported*. Neither is ever counted as $0. | — |
...
| 19–20 | **the last inch:** a three-round chair's summed tokens and its priced cost reach `GigResult.usage` | the loop's accounting has to reach the ledger |
...
| 23 | the invoker honours `ctx.turn_budget`, and hitting it returns the refusal `round_limit` | `turn_budget` was ignored on this path |
