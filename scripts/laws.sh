...
EXPECTED_LAWS="${EXPECTED_LAWS:-4204}"
#  +11 openrouter_reference (new file): the one-line flip (L1a-d), OpenRouter/DeepSeek usage and cost (L2a-f), the
#      provider doc names every variable the door reads (L3). RED at 471169e except L1c and L2b (held; plant-proven).
...
EXPECTED_FILES="${EXPECTED_FILES:-435}"   # + tests/openrouter_reference.test.ts, + tests/every_skill_runs_its_fixtures.test.ts, + tests/node_floor_refuses.test.ts, + tests/spec_reside_drive.test.ts (#537), + tests/a_red_law_names_its_plant.test.ts (#551), + tests/sealed_inputs.test.ts, tests/fan_out.test.ts, tests/completions_seal.test.ts, tests/tier_ladder.test.ts, tests/amend_ladder.test.ts, tests/completions_reasoning_effort.test.ts, tests/bus.test.ts, tests/bus_chair.test.ts, tests/bus_terminal.test.ts, tests/bus_verbs.test.ts, tests/bus_commit.test.ts, tests/code_tools.test.ts, tests/gig_input_validated.test.ts, tests/seat_reads_recorded.test.ts, tests/claude_seat_reads.test.ts, tests/optional_declared_input.test.ts, tests/simulate_names_seats.test.ts
...
EXPECTED_SECURITY_FILES="${EXPECTED_SECURITY_FILES:-4}"   # + openrouter_live_smoke.spec.ts: spends on a real key, skipped (UNVERIFIED) unless COLTRANE_COMPLETIONS_URL is openrouter.ai and a key is set   # + completions_long_wait.spec.ts: the 300s header limit needs a real socket to disprove   # + skill_network_grant.spec.ts: proving a network gate needs a real request, which the root suite forbids
